<template>
    <SynModal v-model="value" title="修改用户信息" width="600" :loading="loading" @on-confirm="confirm">
                ...
                <i-col span="18">
                    ...
                    <i-col span="12">
                        <Form-item label="部门" prop="role_id">
                            <Select ref="roleSelect" v-model="formData.role_id" :clearable="true">
                    ...
                    <i-col span="12">
                        <Form-item label="性别" prop="sex">
                            <Select ref="sexSelect" v-model="formData.sex" :clearable="true">
            ...
            initData () {
                this.$nextTick(function () {
                    for (let key in this.formData) {
                        switch (key) {
                            case 'account':
                                this.formData[key] = this.row.account.split('@')[1];
                                break;
                            case 'password':
                                this.formData[key] = '';
                                break;
                            case 'sex':
                                this.formData.sex = parseInt(this.row.user_info.sex);
                            case 'qq':
                            case 'wechat':
                            case 'address':
                            case 'birthday':
                                this.formData[key] = this.row.user_info[key];
                                break;
                            default:
                                this.formData[key] = this.row[key];
                        }
                    }
                    this.cloneData = deepCopy(this.formData);
                });
            },
            confirm () {
                let data = {};
                for (let key in this.formData) {
                    if (this.formData[key] == this.cloneData[key]) continue;
                    data[key] = this.formData[key];
                }

                this.loading = true;
                this.$store.dispatch('account_user_update', {
                    id: this.row.id,
                    data: data
